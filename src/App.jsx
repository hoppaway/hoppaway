...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ─── HOW IT WORKS STRIP ─── */
  .hiw-strip { max-width: 700px; margin: 0 auto 2rem; padding: 0 1.5rem 0; }
...
const buildAffiliates = (destination, from) => {
  ...
  return [
    ...
    { cat: "🚗 Getting Around", items: [
      { ico:"🚖", name:"Welcome Pickups", desc:"Reliable airport transfers worldwide", cta:"Book transfer →", color:"#1a9e6e", url:"https://tpk.ro/LWp4zUOj" },
    ...
    { cat: "🛡️ Travel Essentials", items: [
      ...
      { ico:"🧳", name:"Radical Storage", desc:"Store your luggage anywhere in the city", cta:"Find storage →", color:"#f5a623", url:"https://radicalstorage.tpk.ro/5hC6QiSS" },
    ]},
  ];
};
                ...
                {cat.items.map((item, ii) => {
                  ...
                  return (
                    <div key={ii} className="packing-item">
                      ...
                      <span className={`packing-item-text ${isChecked ? "checked" : ""}`} onClick={() => toggleCheck(ci, ii)} style={{ cursor: "pointer" }}>
                        {item.text}
                      </span>
                      {aff && !isChecked && (
                        <a className="packing-aff-link" href={aff.url} target="_blank" rel="noopener noreferrer"
                          style={{ color: aff.color, borderColor: aff.color }} onClick={e => e.stopPropagation()}>
                          {aff.label} →
                        </a>
                      )}
                    </div>
  ...
  return (
    <>
      ...
      <div className="packing-page">
        ...
        <h1 className="packing-page-h1">AI packing list<br />for <em>backpackers</em></h1>
        <p className="packing-page-desc">Tell us where you're going and when — get a tailored packing list in seconds. Adapted to your destination, season and duration.</p>
        <div className="packing-form-card">
          <span className="packing-form-label">Your trip</span>
          <div className="fgrid">
            <div className="fg full">
              <label>Where are you going?</label>
              <input type="text" placeholder="Vietnam, Thailand, Portugal..."
                value={form.destination} onChange={e => setForm(p => ({ ...p, destination: e.target.value }))}
                autoCapitalize="words" autoCorrect="off" />
            </div>
            <div className="fg">
              <label>Days</label>
              <input type="number" inputMode="numeric" min="1" max="90" placeholder="7"
          ...
          <button className="btn-gen" onClick={handleGenerate} disabled={!form.destination || loading} style={{ marginTop: "1rem" }}>
        ...
        {result && !loading && (
          <>
                  ...
                  <div key={ci} className="packing-cat">
                    ...
                    {cat.items.map((item, ii) => {
                      ...
                      return (
                        <div key={ii} className="packing-item">
                          <div className={`packing-check ${isChecked ? "checked" : ""}`} onClick={() => toggleCheck(ci, ii)} />
                          <span className={`packing-item-text ${isChecked ? "checked" : ""}`} onClick={() => toggleCheck(ci, ii)} style={{ cursor: "pointer" }}>{item.text}</span>
                          {aff && !isChecked && (
                            <a className="packing-aff-link" href={aff.url} target="_blank" rel="noopener noreferrer"
                              style={{ color: aff.color, borderColor: aff.color }} onClick={e => e.stopPropagation()}>{aff.label} →</a>
                          )}
                        </div>
                      );
                    })}
            ...
            <div className="bmc-bar" style={{ marginTop: "1rem" }}>
              <div className="bmc-text"><strong>☕ Enjoying HoppAway?</strong><br />Free tools like this take time to build. A coffee helps!</div>
              <a className="btn-bmc" href="https://buymeacoffee.com/hoppaway" target="_blank" rel="noopener noreferrer">☕ Buy me a coffee</a>
            </div>
          </>
        )}
      </div>
    ...
    try {
      ...
      if (parsed) {
        ...
        fetch("/api/packing", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ prompt: buildPackingPrompt(parsed.destination || form.destination, form.days, "dry") })
        }).then(r => r.json()).then(data => {
          if (data.text) { const pl = parsePackingList(data.text); if (pl) setPackingData(pl); }
        }).catch(() => {}); // silent fail
